...
export default function SmartInsights({ result, isPro }: SmartInsightsProps) {
  ...
  // Free users: show first insight blurred as a teaser
  const teaserInsight = insights[0];

  return (
    <div className="mt-4 relative">
      {/* Blurred insight teaser */}
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-green-50 to-emerald-50 border border-green-100 p-3">
        ...
        <div className="blur-[6px] select-none pointer-events-none">
          <div className="flex items-start gap-2">
            <span className="text-lg shrink-0">{teaserInsight.emoji}</span>
            <p className="text-sm text-gray-700">{teaserInsight.text}</p>
          </div>
          {insights.length > 1 && (
            <div className="flex items-start gap-2 mt-2">
              <span className="text-lg shrink-0">{insights[1].emoji}</span>
              <p className="text-sm text-gray-700">{insights[1].text}</p>
            </div>
          )}
        </div>
